...
# 1. Update choose_chapter_for_subject to loop deeply and return a tuple path
...
text = re.sub(r'def choose_chapter_for_subject\(data_obj, subject\):.*?return None\s+if choice\.isdigit\(\):\s+index = int\(choice\)\s+if 1 <= index <= len\(chapter_keys\):\s+return chapter_keys\[index - 1\]\s+return\n', new_choose + '\n', text, flags=re.DOTALL)

# 2. Update ensure_structure to handle tuple
...
text = re.sub(r'def ensure_structure\(data_obj, subject, chapter\):.*?data_obj\[subject\]\[chapter\] = \[\]', new_ensure, text, flags=re.DOTALL)

# 3. Update append_and_save to handle tuple
...
text = re.sub(r'def append_and_save\(js_path, original_content, match, data_obj, subject, chapter, entries\):.*?return len\(entries\)', new_append, text, flags=re.DOTALL)


with open('d:/Python_ka_chilla/injector_bot.py', 'w', encoding='utf-8') as f:
    f.write(text)

print("injector_bot.py updated.")
